...
python main.py --opts dataset ${dataset} eval_split ${eval_split} model ${model} problem ${problem} method ${method} num_samples 5 max_batch ${max_batch} batch_size_ip ${batch_size_ip} steps 500 device cuda:3
problem=random_inpainting
python main.py --opts dataset ${dataset} eval_split ${eval_split} model ${model} problem ${problem} method ${method} num_samples 5 max_batch ${max_batch} batch_size_ip ${batch_size_ip} steps 200 device cuda:3
