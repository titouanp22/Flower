...
problem=random_inpainting
python main.py --opts dataset ${dataset} eval_split ${eval_split} model ${model} problem ${problem} method ${method} lmbda 0.01 alpha 0.1 max_batch ${max_batch} batch_size_ip ${batch_size_ip} max_iter 20 # lambda did not match the paper it was 0.001, changed it to the paper one
